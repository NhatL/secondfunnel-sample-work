...
class APITest(APITestCase):
    ...
    def product_search_URL_successful_test(self):
        response = self.client.get('/api/product/search/', {'store': 1, 'url': 'http: //www.facebook.com/product'})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['status'], 'Products with the following ID have been found: 12.')
    ...
    def product_search_sku_successful_test(self):
        response = self.client.get('/api/product/search/', {'store': 1, 'sku': 355353})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['status'], 'Products with the following ID have been found: 12.')
        self.assertEqual(response.data['ids'], [12])
        self.assertEqual(response.data['products'][0]['sku'], u'355353')
    ...
    def product_search_ID_successful_test(self):
        response = self.client.get('/api/product/search/', {'store': 1, 'id': 12})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['status'], 'Products with the following ID have been found: 12.')
        self.assertEqual(response.data['ids'], [12])
        self.assertEqual(response.data['products'][0]['sku'], u'355353')
    ...
    def product_search_multiple_test(self):
        response = self.client.get('/api/product/search/', {'store': 1, 'url': "http: //www.yahoo.com/product"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['status'], 'Products with the following ID have been found: 111 112.')
    ...
    def product_search_URL_unsuccessful_test(self):
        response = self.client.get('/api/product/search/', {'store': 1, 'url': 'http: //www.secondfunnel.com'})
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.data['status'], 'No product was found.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['products'], [])

    def product_search_sku_unsuccessful_test(self):
        response = self.client.get('/api/product/search/', {'store': 1, 'sku': 123456789})
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.data['status'], 'No product was found.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['products'], [])
    ...
    def product_search_ID_unsuccessful_test(self):
        response = self.client.get('/api/product/search/', {'store': 1, 'id': 12356})
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.data['status'], 'No product was found.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['products'], [])
    ...
    def product_search_URL_input_numbers_test(self):
        response = self.client.get('/api/product/search/', {'store': 1, 'url': "12345"})
    ...
    def product_search_URL_input_letters_test(self):
        response = self.client.get('/api/product/search/', {'store': 1, 'url': "asdf1234asf"})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], 'Bad URL input detected.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['products'], [])
    ...
    def product_search_sku_input_URL_test(self):
        response = self.client.get('/api/product/search/', {'store': 1, 'sku': "http: //www.google.com"})
        self.assertEqual(response.status_code, 400)
    ...
    def product_search_sku_input_letters_test(self):
        response = self.client.get('/api/product/search/', {'store': 1, 'sku': "asdf123456"})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], 'Expecting a number as input, but got non-number.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['products'], [])
    ...
    def product_search_ID_input_URL_test(self):
        response = self.client.get('/api/product/search/', {'store': 1, 'id': "http: //www.google.com"})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], 'Expecting a number as input, but got non-number.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['products'], [])
    ...
    def product_search_ID_input_letters_test(self):
        response = self.client.get('/api/product/search/', {'store': 1, 'sku': "asdf1234"})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], 'Expecting a number as input, but got non-number.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['products'], [])

    def product_search_bad_store_test(self):
        response = self.client.get('/api/product/search/', {'store': 1335, 'sku': "asdf1234"})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], 'Store with ID: 1335 has not been found.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['products'], [])

    def product_search_no_input_test(self):
        response = self.client.get('/api/product/search/')
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], 'Missing store ID input.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['products'], [])

    def product_search_multiple_input_test(self):
        response = self.client.get('/api/product/search/', {'store': 1, 'id': 105, 'sku': 1000, 'url': "12345"})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], 'Expecting one of id, ids, name, names, url, urls, sku, skus for search, but got multiple: sku url id.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['products'], [])

    def product_bad_method_test(self):
        response = self.client.post('/api/product/test1/')
    ...
    def content_search_URL_successful_test(self):
        response = self.client.get('/api/content/search/', {'store': 1, 'url': 'http: //www.google.com'})
        self.assertEqual(response.data['status'], 'Contents with the following ID have been found: 6.')
        self.assertEqual(response.data['contents'][0]['id'], 6)
        self.assertEqual(response.data['contents'][0]['store'], 1)
        self.assertEqual(response.data['contents'][0]['name'], 'blah6')
        self.assertEqual(response.data['contents'][0]['description'], "wah wah wah")
        self.assertEqual(response.data['contents'][0]['url'], "/content.jpg")
        self.assertEqual(response.data['contents'][0]['source'], "upload")
        self.assertEqual(response.data['contents'][0]['source_url'], "http: //www.google.com")
        self.assertEqual(response.data['contents'][0]['tagged_products'], [])
        self.assertEqual(response.data['contents'][0]['attributes'], {})
        self.assertEqual(response.data['contents'][0]['status'], "needs-review")
        self.assertEqual(response.data['ids'], [6])

    def content_search_ID_successful_test(self):
        response = self.client.get('/api/content/search/', {'store': 14, 'id': 16})
        self.assertEqual(response.data['status'], 'Contents with the following ID have been found: 16.')
        self.assertEqual(response.data['contents'][0]['id'], 16)
        self.assertEqual(response.data['contents'][0]['store'], 14)
        self.assertEqual(response.data['contents'][0]['name'], 'blah16')
        self.assertEqual(response.data['contents'][0]['description'], "blah blah blah")
        self.assertEqual(response.data['contents'][0]['url'], "/content2.jpg")
        self.assertEqual(response.data['contents'][0]['source'], "upload")
        self.assertEqual(response.data['contents'][0]['source_url'], "http: //www.facebook.com")
        self.assertEqual(response.data['contents'][0]['tagged_products'], [])
        self.assertEqual(response.data['contents'][0]['attributes'], {})
        self.assertEqual(response.data['contents'][0]['status'], "approved")
        self.assertEqual(response.data['ids'], [16])

    def content_search_multiple_test(self):
        response = self.client.get('/api/content/search/', {'store': 14, 'url': "http: //www.facebook.com"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['status'], 'Contents with the following ID have been found: 535 534 16.')
        self.assertEqual(response.data['ids'], [535, 534, 16])

        self.assertEqual(response.data['contents'][0]['status'], u'approved')
        self.assertEqual(response.data['contents'][0]['tagged_products'], [])
        self.assertEqual(response.data['contents'][0]['description'], u'blah blah blah535')
        ...
        self.assertEqual(response.data['contents'][0]['attributes'], {})
        self.assertEqual(response.data['contents'][0]['id'], 535)
        self.assertEqual(response.data['contents'][0]['store'], 14)
        self.assertEqual(response.data['contents'][0]['name'], u'blah535')
        ...
        self.assertEqual(response.data['contents'][2]['description'], u'blah blah blah')
        self.assertEqual(response.data['contents'][2]['url'], u'/content2.jpg')
        ...
        self.assertEqual(response.data['contents'][2]['id'], 16)
        self.assertEqual(response.data['contents'][2]['store'], 14)
        self.assertEqual(response.data['contents'][2]['name'], u'blah16')

    def content_search_URL_unsuccessful_test(self):
        response = self.client.get('/api/content/search/', {'store': 1, 'url': 'http: //www.secondfunnel.com'})
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.data['status'], 'No content was found.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['contents'], [])

    def content_search_ID_unsuccessful_test(self):
        response = self.client.get('/api/content/search/', {'store': 1, 'id': 12356})
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.data['status'], u'No content was found.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['contents'], [])

    def content_search_URL_input_numbers_test(self):
        response = self.client.get('/api/content/search/', {'store': 1, 'url': "12345"})
        self.assertEqual(response.status_code, 400)
    ...
    def content_search_URL_input_letters_test(self):
        response = self.client.get('/api/content/search/', {'store': 1, 'url': "asdf1234asf"})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], u'Bad URL input detected.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['contents'], [])
    ...
    def content_search_ID_input_URL_test(self):
        response = self.client.get('/api/content/search/', {'store': 1, 'id': "http: //www.google.com"})
        self.assertEqual(response.status_code, 400)
    ...
    def content_search_ID_input_letters_test(self):
        response = self.client.get('/api/content/search/', {'store': 1, 'id': "asdf1234"})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], u'Expecting a number as input, but got non-number.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['contents'], [])

    def content_search_bad_store_test(self):
        response = self.client.get('/api/content/search/', {'store': 13531, 'id': "1"})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], u'Store with ID: 13531 has not been found.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['contents'], [])
    ...
    def content_search_no_input_test(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], u'Missing store ID input.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['contents'], [])

    def content_search_multiple_input_test(self):
        response = self.client.get('/api/content/search/', {'store': 1, 'id': 105, 'name': 1000, 'url': "12345"})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], u'Expecting one of id, ids, name, or url for search, but got multiple: url id name.')
        self.assertEqual(response.data['ids'], [])
        self.assertEqual(response.data['contents'], [])
    ...
    def page_add_product_successful_test(self):
        response = self.client.post('/api/page/8/remove/', {'type': 'product', 'id': 3})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['status'], 'Product with ID: 3, Name: Default has been removed.')

        response = self.client.post('/api/page/8/add/', {'type': 'product', 'id': 3})
        self.assertEqual(response.status_code, 200)
        ...
        self.assertEqual(response.data['tile']['products'][0]['sku'], u'1234')
        self.assertEqual(response.data['tile']['products'][0]['description'], u'default default')
        self.assertEqual(response.data['tile']['products'][0]['url'], u'http: //www.google.com/product')
        self.assertEqual(response.data['tile']['products'][0]['price'], u'19.99')
        self.assertEqual(response.data['tile']['products'][0]['last_scraped_at'], None)
        self.assertEqual(response.data['tile']['products'][0]['similar_products'], [])
        self.assertEqual(response.data['tile']['products'][0]['currency'], u'$')
        self.assertEqual(response.data['tile']['products'][0]['in_stock'], True)
        self.assertEqual(response.data['tile']['products'][0]['default_image'], 4)
        self.assertEqual(response.data['tile']['products'][0]['details'], u'<li>blah blah</li>')
        self.assertEqual(response.data['tile']['products'][0]['attributes'], {})
        self.assertEqual(response.data['tile']['products'][0]['id'], 3)
        self.assertEqual(response.data['tile']['products'][0]['store'], 1)
        self.assertEqual(response.data['tile']['products'][0]['name'], u'Default')
    ...
    def page_add_product_successful_category_priority_test(self):
        ...
        response = self.client.post('/api/page/8/add/', {'type': 'product', 'id': 3, 'category': "TestCategory", 'priority': 100})
        self.assertEqual(response.status_code, 200)
    ...
    def page_add_product_successful_category_test(self):
        ...
        response = self.client.post('/api/page/8/add/', {'type': 'product', 'id': 3, 'category': "TestCategory"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['status'], u'Product with ID: 3, Name: Default has been added.')
        self.assertEqual(response.data['id'], 16)
        self.assertEqual(response.data['tile']['feed'], 9)
    ...
    def page_add_product_successful_priority_test(self):
        ...
        response = self.client.post('/api/page/8/add/', {'type': 'product', 'id': 3, 'priority': 1000})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['status'], u'Product with ID: 3, Name: Default has been added.')
        self.assertEqual(response.data['id'], 16)
        self.assertEqual(response.data['tile']['feed'], 9)
    ...
    def page_add_content_successful_test(self):
        response = self.client.post('/api/page/8/remove/', {'type': 'content', 'id': 6})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['status'], 'Content with ID: 6 has been removed.')

        response = self.client.post('/api/page/8/add/', {'type': 'content', 'id': 6})
        self.assertEqual(response.status_code, 200)
    ...
    def page_add_content_successful_category_test(self):
        ...
        response = self.client.post('/api/page/8/add/', {'type': 'content', 'id': 6, 'category': "TestCategory"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['status'], 'Content with ID: 6 has been added.')
        self.assertEqual(response.data['id'], 16)
        self.assertEqual(response.data['tile']['feed'], 9)
    ...
    def page_add_product_unsuccessful_test(self):
        response = self.client.post('/api/page/8/add/', {'type': 'product', 'id': 1000})
        self.assertEqual(response.status_code, 404)
    ...
    def page_add_content_unsuccessful_test(self):
        response = self.client.post('/api/page/8/add/', {'type': 'content', 'id': 1000})
        self.assertEqual(response.status_code, 404)
    ...
    def page_add_no_input_test(self):
        response = self.client.post('/api/page/8/add/')
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], u"Missing 'id' field from input.")

    def page_add_bad_type_test(self):
        response = self.client.post('/api/page/8/add/', {'type': 'product3', 'id': 6, 'priority': 100})
        self.assertEqual(response.status_code, 400)
    ...
    def page_add_bad_ID_test(self):
        response = self.client.post('/api/page/8/add/', {'type': 'product', 'id': 'asdf'})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], u'Expecting a number as input, but got non-number.')

    def page_add_bad_category_test(self):
        response = self.client.post('/api/page/8/add/', {'type': 'product', 'id': 6, 'category': "TestFaulty"})
        self.assertEqual(response.status_code, 400)
    ...
    def page_add_bad_priority_test(self):
        response = self.client.post('/api/page/8/add/', {'type': 'product', 'id': 6, 'priority': "TestFaulty"})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], u"Priority 'TestFaulty' is not a number.")

    def page_add_missing_type_test(self):
        response = self.client.post('/api/page/8/add/', {'typo': 'product', 'id': 6})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], u"Missing 'type' field from input.")

    def page_add_missing_ID_test(self):
        response = self.client.post('/api/page/8/add/', {'type': 'product', 'id2': 6})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], u"Missing 'id' field from input.")

    def page_add_bad_page_ID_test(self):
        response = self.client.post('/api/page/10000/add/', {'type': 'product', 'id': 6})
        self.assertEqual(response.status_code, 404)
    ...
    def page_remove_product_unsuccessful_test(self):
        response = self.client.post('/api/page/8/remove/', {'type': 'product', 'id': 100000})
        self.assertEqual(response.status_code, 404)
    ...
    def page_remove_product_category_successful_test(self):
        ...
        response = self.client.post('/api/page/8/add/', {'type': 'product', 'id': 3, 'category': 'TestCategory2'})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['status'], 'Product with ID: 3, Name: Default has been added.')
        self.assertEqual(response.data['id'], 16)
        self.assertEqual(response.data['tile']['feed'], 9)
        ...
        response = self.client.post('/api/page/8/remove/', {'type': 'product', 'id': 3, 'category': 'TestCategory2'})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['status'], 'Product with ID: 3, Name: Default has been removed.')

    def page_remove_product_category_unsuccessful_test(self):
        response = self.client.post('/api/page/8/remove/', {'type': 'product', 'id': 3, 'category': 'TestCategory3'})
        self.assertEqual(response.status_code, 400)
    ...
    def page_remove_product_category_unsuccessful2_test(self):
        response = self.client.post('/api/page/8/remove/', {'type': 'product', 'id': 3, 'category': 'TestCategory'})
        self.assertEqual(response.status_code, 400)
    ...
    def page_remove_content_category_successful_test(self):
        ...
        response = self.client.post('/api/page/8/remove/', {'type': 'content', 'id': 6, 'category': 'TestCategory'})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['status'], 'Content with ID: 6 has been removed.')

    def page_remove_content_category_unsuccessful_test(self):
        response = self.client.post('/api/page/8/remove/', {'type': 'content', 'id': 6, 'category': 'TestCategory2'})
        self.assertEqual(response.status_code, 400)
    ...
    def page_remove_content_category_unsuccessful2_test(self):
        response = self.client.post('/api/page/8/remove/', {'type': 'content', 'id': 6, 'category': 'TestCategory3'})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], "Category 'TestCategory3' not found for store 'MyStore'.")
    ...
    def page_remove_content_unsuccessful_test(self):
        response = self.client.post('/api/page/8/remove/', {'type': 'content', 'id': 100000})
        self.assertEqual(response.status_code, 404)
    ...
    def page_remove_no_input_test(self):
        response = self.client.post('/api/page/8/remove/')
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], u"Missing 'id' field from input.")

    def page_remove_bad_type_test(self):
        response = self.client.post('/api/page/8/remove/', {'type': 'product3', 'id': 100000, 'priority': 100})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], u"Type 'product3' is not a valid type (content/product only).")

    def page_remove_bad_ID_test(self):
        response = self.client.post('/api/page/8/remove/', {'type': 'product', 'id': 'asdf'})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], u'Expecting a number as input, but got non-number.')

    def page_remove_missing_type_test(self):
        response = self.client.post('/api/page/8/remove/', {'typo': 'product', 'id': 5})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], u"Missing 'type' field from input.")

    def page_remove_missing_ID_test(self):
        response = self.client.post('/api/page/8/remove/', {'type': 'product', 'id2': 'ID'})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['status'], u"Missing 'id' field from input.")

    def page_remove_bad_page_ID_test(self):
        response = self.client.post('/api/page/10000/remove/', {'type': 'product', 'id': 6})
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.data['status'], u"Page with ID: 10000 not found.")
    ...
    def tile_test(self):
        ...
        response = self.client.get('/api/tile?page=8')
        ...
        self.assertEqual(tile0['template'], u'product')
        self.assertEqual(tile0['products'], [ProductSerializer(Product.objects.get(pk=3)).data])
        self.assertEqual(tile0['content'], [])
        ...
        self.assertEqual(tile1['template'], u'content')
        self.assertEqual(tile1['products'], [])
        self.assertEqual(tile0['content'], [ContentSerializer(Content.objects.get(pk=6)).data])
        ...
        self.assertEqual(tile2['template'], u'product')
        self.assertEqual(tile2['products'], [ProductSerializer(Product.objects.get(pk=111)).data])
        self.assertEqual(tile2['content'], [])
        ...
        self.assertEqual(tile3['template'], u'content')
        self.assertEqual(tile3['products'], [])
        self.assertEqual(tile3['content'], [6])
        ...
        self.assertEqual(tile4['template'], u'product')
        self.assertEqual(tile4['products'], [112])
        self.assertEqual(tile4['content'], [])
    ...
    def tile_single_test(self):
        ...
        response = self.client.get('/api/tile/11/')
        tile = response.data
        self.assertEqual(tile['id'], 11)
        self.assertEqual(tile['feed'], 9)
        self.assertEqual(tile['template'], u'content')
        ...
        self.assertEqual(tile['content'][0]['status'], u'needs-review')
        self.assertEqual(tile['content'][0]['tagged_products'], [])
        self.assertEqual(tile['content'][0]['description'], u'wah wah wah')
        self.assertEqual(tile['content'][0]['url'], u'/content.jpg')
        self.assertEqual(tile['content'][0]['source_url'], u'http: //www.google.com')
        self.assertEqual(tile['content'][0]['source'], u'upload')
        self.assertEqual(tile['content'][0]['attributes'], {})
        self.assertEqual(tile['content'][0]['id'], 6)
        self.assertEqual(tile['content'][0]['store'], 1)
        self.assertEqual(tile['content'][0]['name'], u'blah6')
    ...
    def tile_change_prio_test(self):
        ...
        response = self.client.patch('/api/tile', json.dumps([{"id": 10, "priority": 100000}]), content_type='application/json')
        self.assertEqual(response.status_code, 200)
        t = Tile.objects.get(pk=10)
        self.assertEqual(response.data[0]['id'], 10)
        self.assertEqual(response.data[0]['feed'], 9)
        self.assertEqual(response.data[0]['template'], u'product')
        self.assertEqual(response.data[0]['products'], [112])
        self.assertEqual(response.data[0]['content'], [])
        self.assertEqual(response.data[0]['priority'], 100000)
        self.assertEqual(response.data[0]['clicks'], 0)
        self.assertEqual(response.data[0]['views'], 0)
        self.assertEqual(response.data[0]['placeholder'], False)
        self.assertEqual(response.data[0]['in_stock'], True)
        self.assertEqual(response.data[0]['attributes'], {})
        self.assertEqual(t.priority, 100000)

        response = self.client.patch('/api/tile/10/', {"priority": 100001})
        self.assertEqual(response.status_code, 200)
        t = Tile.objects.get(pk=10)
        self.assertEqual(response.data['id'], 10)
        self.assertEqual(response.data['feed'], 9)
        self.assertEqual(response.data['template'], u'product')
        self.assertEqual(response.data['products'], [112])
        self.assertEqual(response.data['content'], [])
        self.assertEqual(response.data['priority'], 100001)
        self.assertEqual(response.data['clicks'], 0)
        self.assertEqual(response.data['views'], 0)
        self.assertEqual(response.data['placeholder'], False)
    ...
    def tile_change_prio_bulk_test(self):
        ...
        response = self.client.patch('/api/tile', json.dumps([{"id": 10, "priority": 100000}, {"id": 11, "priority": 101}]), content_type='application/json')
        self.assertEqual(response.status_code, 200)
        t = Tile.objects.get(pk=10)
        self.assertEqual(response.data[0]['id'], 10)
        self.assertEqual(response.data[0]['feed'], 9)
        ...
        self.assertEqual(response.data[1]['id'], 11)
        self.assertEqual(response.data[1]['feed'], 9)
        self.assertEqual(response.data[1]['template'], u'content')
        self.assertEqual(response.data[1]['products'], [])
        self.assertEqual(response.data[1]['content'], [6])
        self.assertEqual(response.data[1]['priority'], 101)
        self.assertEqual(response.data[1]['clicks'], 0)
        self.assertEqual(response.data[1]['views'], 0)
        self.assertEqual(response.data[1]['placeholder'], False)
        self.assertEqual(response.data[1]['in_stock'], True)
        self.assertEqual(response.data[1]['attributes'], {})
    ...
    def tile_change_prio_wrongID_test(self):
        ...
        response = self.client.patch('/api/tile/55/', {'priority': 1})
        self.assertEqual(response.status_code, 404)
        t = Tile.objects.filter(pk=55)
        self.assertEqual(response.data['detail'], 'Not found.')
        self.assertEqual(list(t), [])

        response = self.client.patch('/api/tile', json.dumps([{"id": 55, "priority": 1}]), content_type='application/json')
        self.assertEqual(response.status_code, 400)
        t = Tile.objects.filter(pk=55)
    ...
    def tile_change_prio_bad_inputs_test(self):
        ...
        response = self.client.patch('/api/tile/test/', {'priority': 1})
        self.assertEqual(response.status_code, 404)
    ...
    def tile_change_prio_bad_inputs2_test(self):
        ...
        response = self.client.patch('/api/tile/10/', {'priority': 'test'})
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['priority'], [u'A valid integer is required.'])

        response = self.client.patch('/api/tile', json.dumps([{"id": 10, "priority": "test"}]), content_type='application/json')
        self.assertEqual(response.status_code, 400)
    ...
    def tile_error_test(self):
        ...
        response = self.client.get('/api/tile/999/')
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.data, {u'detail': u'Not found.'})
        self.assertEqual(response.data[u'detail'], u'Not found.')
    ...
    def tile_delete_successful_test(self):
        ...
        response = self.client.delete('/api/tile/10/')
        self.assertEqual(response.status_code, 200)
    ...
    def tile_delete_unsuccessful_test(self):
        ...
        response = self.client.delete('/api/tile/40000/')
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.data, {u'detail': u'Not found.'})
        self.assertEqual(response.data[u'detail'], u'Not found.')
